"""V3 backbone; V3.1 and R1 reuse this disclosed architecture family."""
from .decoder import Block, Decoder
from ..mla import MLA
from ..moe import Expert, MoE


def build(vocab=32, width=32, layers=2, rotary=None):
    if layers < 1:
        raise ValueError('Need at least one layer')
    blocks = []
    for layer in range(layers):
        attention = MLA(width=width, rotary=rotary)
        ffn = Expert(width, 3*width) if layer == 0 else MoE(
            width, hidden=width//2, routed=8, shared=1, top_k=2,
            score='sigmoid', normalize=True, bias_rate=.001,
            groups=2, top_groups=1, group_score='sum2', route_scale=2.5)
        blocks.append(Block(width, attention, ffn))
    return Decoder(vocab, width, blocks)
